...
def speedCorrection(dummy):
	...
	global pidL,pidR,freq_L,freq_R, dacL, dacR
...
def isr_motorA(dummy):
	global A_count
	global dr, r, flag2,flaga,left_dist, block_dist
	A_count+=1
	flaga=True
	dr+=((2*math.pi*r)/30)
	flag2=True
...
def isr_speed_timer(dummy):
	...
	global B_count
	global B_speed
	global count,flag,rightflag,leftflag,stopflag,block_dist
	A_speed=A_count
	B_speed=B_count
	A_count=0
	B_count=0
	...
	if(rightflag==2 and leftflag==2 and stopflag==2 and startflag==2):		#flags to ensure the self correction happens only when behicle is moving straight
		#print("enter")
...
def start():
	...
	stop()
	#print("start")
	'''global flagspeed,init_dacA,init_dacB,brake_status,status,count,stopflag,flag,flag3,startflag,block_dist,total_dist, ct
	block_dist=0
	if(~(brake_status)):
		if(ct==1):
			#print("start enter")
		...
		while(block_dist<total_dist):
			startflag=2
			if(flagspeed==2 and total_dist<50):
				#print(count,flag,flag3)
				#------------------------------slow to high speed------------------------------------------------------------------
				if(count==30 and flag and flag3):
					init_dacA+=70
					init_dacB+=70
					flag=False
				if(count==60 and flag and flag3):
					init_dacA+=70
					init_dacB+=70
					flag=False
				if(count==90 and flag and flag3):
					init_dacA+=70
					init_dacB+=70
					flag=False
				if(count==120 and flag and flag3):
					init_dacA+=70
					init_dacB+=70
					flag=False
				if(count==150 and flag and flag3):
					init_dacA+=40
					init_dacB+=40
					flag=False
				if(count==180 and flag and flag3):
					init_dacA+=30
					init_dacB+=30
					flag=False
				if(count==210 and flag and flag3):
					init_dacA+=30
					init_dacB+=30
					flag=False
				if(count==240 and flag and flag3):
...
def stop():
	#print("stop")
	global stopflag,A_speed,B_speed,init_dacA,init_dacB,block_dist
	stopflag=3
